...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		...
		width = computeDimensions()[0];
		height = computeDimensions()[1];
		skirt = compSkirt();

	}

	private int[] compSkirt() {
		...
		for(int i=0; i< body.length; i++ ){
			if(arr[body[i].x] == -1){
				arr[body[i].x] = body[i].y;
			} else if (arr[body[i].x] > body[i].y )
				arr[body[i].x] = body[i].y;
		}
		return arr;
	}
	...
	private int [] computeDimensions() {
		...
		for(int i=0; i< body.length; i++){
			if (maxx==-1){
				...
				if (body[i].x > maxx) maxx = body[i].x;
				if (body[i].x < minx) minx = body[i].x;
				if (body[i].y > maxy) maxy = body[i].y;
				if (body[i].y < miny) miny = body[i].y;
			}
		}
		int[] arr= new int[2];
		if(maxx==-1){
	...
	*/
	...
	public Piece computeNextRotation() {
		if(width==0 & height==0) return new Piece("");
		StringBuilder str= new StringBuilder("");

		for(int i=0; i< body.length; i++){
			str.append( Integer.toString(height - 1 -body[i].y) + ' ' + Integer.toString(body[i].x) + "  ");
		}
		str.delete(str.length()-2, str.length());
		Piece ans = new Piece(str.toString());
		return ans;
	}
	...
	*/
	public boolean equals(Object obj) {
		...
		for(TPoint pt : body){
			points.add(pt.toString());
		}
		for (TPoint pt : other.body){
			if (!points.contains(pt.toString())) {
				return false;
			}
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		Piece cur = root;
		while(!cur.computeNextRotation().equals(root)){
